...
¡Cuidado! Esta distribución de nThreads no es compatible con procesadores
Arm.  Si Ud. posee un Mac con Arm contácteme para ver en qué computador
resuelve esta tarea.
...
Para compilar sus tareas previas copie suma.c, disk.c y batch.c a este
directorio y cambie:
...
difíciles de diagnosticar, *especialmenter* si esa función invoca malloc/free.
Funciones como strlen, strcpy, etc. sí se pueden usar porque no invocan
malloc/free.

Para compilar por ejemplo batch.c ejecute alguno de estos comandos:

make PROB=batch run-san
make PROB=batch run-g
make PROB=batch run

El primer comando genera el binario batch.bin-san, el segundo batch.bin-g y
el tercero batch.bin.  Ejecútelos con la opción -h para recibir explicaciones
sobre las opciones para ejecutar con distintos schedulings.  Por ejemplo:

./batch.san -h

Use PROB=suma para probar su tarea 1.  Considere que no pasará el test
de eficiencia con make PROB=suma run, porque su tarea se ejecutará con
un solo core virtual (un solo pthread).  Ejecútelo manualmente con varios
cores.  Por ejemplo:

./suma.bin -ncores 4

Use PROB=disk para su tarea 2.

-----------------------------------------------

Instrucciones para probar su tarea 4

Ud. debe programar la función solicitada en el archivo nexchange.c.
...
archivo nexchange.zip que contiene nexchange.c, con su solución, y
...
Si lo prefiere, lance codeblocks y abra el archivo nexchange.cbp.
...
quad-core, vaya a Project -> Set programs' argumets, seleccione un target y
...
Entregue por U-cursos el archivo nexchange.zip
